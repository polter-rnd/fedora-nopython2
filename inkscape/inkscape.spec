...
# Remove all python and ruby extensions
sh %{SOURCE3} "$RPM_BUILD_ROOT%{_datadir}/%{name}/extensions" python
sh %{SOURCE3} "$RPM_BUILD_ROOT%{_datadir}/%{name}/extensions" ruby

# Remove all unneeded scripts
find "$RPM_BUILD_ROOT%{_datadir}/%{name}/extensions" -type f -name *.py -delete
find "$RPM_BUILD_ROOT%{_datadir}/%{name}/extensions" -type f -name *.rb -delete

# Remove all unneeded directories
rm -rf $RPM_BUILD_ROOT%{_datadir}/%{name}/extensions/alphabet_soup
rm -rf $RPM_BUILD_ROOT%{_datadir}/%{name}/extensions/Barcode
rm -rf $RPM_BUILD_ROOT%{_datadir}/%{name}/extensions/ink2canvas
rm -rf $RPM_BUILD_ROOT%{_datadir}/%{name}/extensions/Poly3DObjects
...
- Disabled gnome-vfs
- Added libcdr and libvisio build requirements
